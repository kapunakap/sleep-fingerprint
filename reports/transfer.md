...
- `metrics/transfer_28643153/age_predictions.csv`
...
The script verifies the Figshare article license, streams only the public BCG samples needed for the frozen protocol, uses participant-only folds, writes derived metrics/CSVs/plots, and never commits raw target data. The committed per-subject age predictions are generated by this same deterministic protocol so the failed age-regression result can be independently audited rather than existing only as aggregate metrics.
